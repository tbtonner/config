...
var require_semver = __commonJS({
  "node_modules/semver/semver.js"(exports2, module2) {
    ...
    Range6.prototype.test = function(version2) {
      if (!version2) {
        return false;
      }
      if (typeof version2 === "string") {
...
var require_rimraf = __commonJS({
  "node_modules/rimraf/rimraf.js"(exports2, module2) {
        ...
        options.unlink(p, (er2) => {
          if (er2) {
            if (er2.code === "ENOENT")
              return cb(null);
            if (er2.code === "EPERM")
              return isWindows3 ? fixWinEPERM(p, options, er2, cb) : rmdir(p, options, er2, cb);
            if (er2.code === "EISDIR")
    ...
    var rmkidsSync = (p, options) => {
      ...
      const retries = isWindows3 ? 100 : 1;
      let i = 0;
      do {
        let threw = true;
        try {
...
var require_command_exists = __commonJS({
  "node_modules/command-exists/lib/command-exists.js"(exports2, module2) {
    ...
    var accessSync = fs3.accessSync;
    var constants = fs3.constants || fs3;
...
var require_lib = __commonJS({
  "node_modules/isbinaryfile/lib/index.js"(exports2) {
    ...
    var statAsync = util_1.promisify(fs3.stat);
    var openAsync = util_1.promisify(fs3.open);
    var closeAsync = util_1.promisify(fs3.close);
    var MAX_BYTES = 512;
    var Reader = class {
      ...
      return __awaiter2(this, void 0, void 0, function* () {
        if (isString(file)) {
          ...
          return new Promise((fulfill, reject) => {
            fs3.read(fileDescriptor, allocBuffer, 0, MAX_BYTES, 0, (err, bytesRead, _) => {
    ...
    function isBinaryFileSync(file, size) {
      if (isString(file)) {
        ...
        isStatFile(stat);
        const fileDescriptor = fs3.openSync(file, "r");
        const allocBuffer = Buffer.alloc(MAX_BYTES);
        const bytesRead = fs3.readSync(fileDescriptor, allocBuffer, 0, MAX_BYTES, 0);
        fs3.closeSync(fileDescriptor);
...
var HelperVimEvents = class {
  ...
  async register(context2) {
    ...
    context2.subscriptions.push(import_coc7.Disposable.create(() => {
      eventsModule.deactivate.call(this.augroupName).catch(this.helperLogger.error);
    }));
    context2.subscriptions.push(import_coc7.commands.registerCommand(this.commandName, helperLogger.asyncCatch((event, ...args) => this.events.fire(event, ...args)), void 0, true));
  }
};
...
var WinLayoutFinder = class {
  ...
  static async create(tabnr) {
    ...
    const root2 = await import_coc13.workspace.nvim.call("winlayout", args);
...
async function activateHelper(context2, options = {}) {
  ...
  if ((_c = options.events) !== null && _c !== void 0 ? _c : true) {
    await helperVimEvents.register(context2);
  }
  try {
...
var config = import_coc15.workspace.getConfiguration("explorer");
var configLocal = (resource = generateUri2(import_coc15.workspace.cwd)) => import_coc15.workspace.getConfiguration("explorer", resource);
var bufferTabOnly = () => {
...
var nodejsModuleTrash = async (paths) => {
  ...
  const scriptPath = import_path.default.join(__dirname, "../cli/trash.mjs");
...
var fsOpen = (0, import_util12.promisify)(import_fs.default.open);
var fsClose = (0, import_util12.promisify)(import_fs.default.close);
var fsTouch = async (path2) => await fsClose(await fsOpen(path2, "w"));
var fsMkdirp = import_make_dir.default;
var fsReaddir = (0, import_util12.promisify)(import_fs.default.readdir);
var fsReadlink = (0, import_util12.promisify)(import_fs.default.readlink);
var fsAccess = (path2, mode) => new Promise((resolve) => {
  import_fs.default.access(path2, mode, (err) => {
    err ? resolve(false) : resolve(true);
  });
});
var fsWriteFile = (0, import_util12.promisify)(import_fs.default.writeFile);
var fsReadFile = (0, import_util12.promisify)(import_fs.default.readFile);
var fsExists = fsAccess;
var fsStat = (0, import_util12.promisify)(import_fs.default.stat);
var fsLstat = (0, import_util12.promisify)(import_fs.default.lstat);
var fsCopyFile = (0, import_util12.promisify)(import_fs.default.copyFile);
var fsRename = (0, import_util12.promisify)(import_fs.default.rename);
var fsRimraf = (0, import_util12.promisify)(import_rimraf.default);
  ...
  if (lstat.isDirectory()) {
    ...
    for (const filename of filenames) {
      await fsCopyFileRecursive(
        import_path2.default.join(sourcePath, filename),
        import_path2.default.join(targetPath, filename)
      );
    }
  } else {
...
async function fsMergeDirectory(sourceDir, targetDir, action) {
  ...
  for (const filename of filenames) {
    const sourcePath = import_path2.default.join(sourceDir, filename);
...
async function overwritePrompt(promptText, paths, action) {
  ...
  const finalAction = async (source, target) => {
    await fsMkdirp(import_path2.default.dirname(target));
        ...
        await prompt_({
          ...
          "one by one": async () => {
            ...
            const paths2 = files.map((source) => ({
              source,
              target: import_path2.default.join(targetPath, import_path2.default.basename(source))
            }));
...
function readFileLines(fullpath, start, end) {
  ...
  const res = [];
  const stream = import_fs.default.createReadStream(fullpath, { encoding: "utf8" });
...
function getExtensions(filename) {
  const parts = import_path3.default.basename(filename).split(".");
  const [basename, ...extensions3] = parts;
  if (basename) {
    return { basename, extensions: extensions3 };
  } else {
    const [basename2, ...extensions22] = extensions3;
    return {
...
function normalizePath(path2) {
  let _path = import_path3.default.normalize(path2);
  if (_path[0] === "~") {
    _path = import_path3.default.join(import_os2.default.homedir(), _path.slice(1));
  }
...
function isParentFolder(folder, filepath) {
  let finalFolder = normalizePath(import_path3.default.resolve(folder));
  const finalFilepath = normalizePath(import_path3.default.resolve(filepath));
  if (finalFolder === "//")
    finalFolder = "/";
  if (finalFolder.endsWith(import_path3.default.sep))
    return finalFilepath.startsWith(finalFolder);
  return finalFilepath.startsWith(finalFolder) && finalFilepath[finalFolder.length] === import_path3.default.sep;
}
...
function supportedFloat() {
  return import_coc16.workspace.floatSupported;
}
function supportedNvimFloating() {
  return import_coc16.workspace.isNvim && supportedFloat();
}
async function enableWrapscan() {
  const wrapscan = await import_coc16.workspace.nvim.getOption("wrapscan");
  return !!wrapscan;
}
...
async function displayWidth2(str) {
  return await import_coc16.workspace.nvim.call("strdisplaywidth", [str]);
}
async function displaySlice(str, start, end) {
  return await import_coc16.workspace.nvim.call("coc_explorer#util#strdisplayslice", [
    str,
    start,
    end ?? void 0
  ]);
...
function closeWinByBufnrNotifier(bufnrs) {
  return Notifier.create(() => {
    import_coc16.workspace.nvim.call("coc_explorer#util#close_win_by_bufnr", bufnrs, true);
  });
}
async function winnrByBufnr(bufnr) {
  ...
  return import_coc16.workspace.nvim.call("bufwinnr", bufnr).then((winnr) => {
    if (winnr > 0) {
      return winnr;
    } else {
      return void 0;
...
async function winidByWinnr(winnr) {
  ...
  const winid = await import_coc16.workspace.nvim.call("win_getid", winnr);
  if (winid >= 0) {
    return winid;
  } else {
    return void 0;
...
async function winByWinid(winid) {
  if (winid) {
    return import_coc16.workspace.nvim.createWindow(winid);
  } else {
    return void 0;
  }
}
async function bufnrByWinnrOrWinid(winnrOrWinid) {
  ...
  const bufnr = await import_coc16.workspace.nvim.call("winbufnr", winnrOrWinid);
  if (bufnr >= 0) {
    return bufnr;
  } else {
    return void 0;
...
async function winidsByBufnr(bufnr) {
  return await import_coc16.workspace.nvim.call("win_findbuf", [bufnr]);
}
async function winidsByBufnrInCurTab(bufnr) {
  const tabpage = await import_coc16.workspace.nvim.tabpage;
  const wins = await tabpage.windows;
  const winidsOfTab = wins.map((win) => win.id);
  const allWinids = await import_coc16.workspace.nvim.call("win_findbuf", [
    bufnr
  ]);
  return allWinids.filter((winid) => winidsOfTab.includes(winid));
}
async function leaveEmptyInWinids(winids) {
  const curWinid = await import_coc16.workspace.nvim.call("win_getid", []);
  if (!winids.length) {
    return;
  }
  import_coc16.workspace.nvim.pauseNotification();
  for (const winid of winids) {
    import_coc16.workspace.nvim.call("win_gotoid", [winid], true);
    import_coc16.workspace.nvim.command("enew", true);
    if (import_coc16.workspace.isVim) {
      import_coc16.workspace.nvim.command("redraw", true);
    }
  }
  import_coc16.workspace.nvim.call("win_gotoid", [curWinid], true);
  await import_coc16.workspace.nvim.resumeNotification();
}
async function currentBufnr() {
  return import_coc16.workspace.nvim.call("bufnr");
}
...
async function getFloatInputApi() {
  if (!floatInputExt) {
    floatInputExt = import_coc17.extensions.all.find((e) => e.id === "coc-floatinput");
  }
...
async function vimPrompt(msg, choices, defaultChoice) {
  ...
  const result = await import_coc17.workspace.nvim.call("confirm", [
    msg,
    choices.map((choice) => {
...
async function vimInput(prompt2, defaultInput = "", completion = void 0) {
  return import_coc17.workspace.nvim.callAsync("coc#util#with_callback", [
    "input",
...
async function selectWindowsUI(config4, sourceType, {
  ...
  const winnr = await import_coc17.workspace.nvim.call("coc_explorer#select_wins#start", [
...
function subjectToHook(observable2) {
  return (fn) => {
    ...
    return import_coc18.Disposable.create(() => sub.unsubscribe());
  };
}
...
var ProxyList = class extends import_coc19.BasicList {
  constructor(init) {
    super(import_coc19.workspace.nvim);
    this.name = init.name;
...
async function startCocList(explorer, list, arg, listArgs = []) {
  ...
  const shownExplorerEmitter = new import_coc19.Emitter();
  const listDisposable = import_coc19.listManager.registerList(list);
  await nvim.command(`CocList ${listArgs.join(" ")} ${list.name}`);
  listDisposable.dispose();
  return {
    async waitExplorerShow() {
      await bufManager.waitReload();
      const eventDisposable = import_coc19.events.on("BufEnter", async () => {
...
var actionListMru = import_coc20.workspace.createMru("explorer-actions");
...
var KeyMapping = class {
  ...
  async filterEscForVim(keys2) {
    if (import_coc21.workspace.isVim && !await import_coc21.workspace.nvim.call("has", ["gui_running"])) {
      keys2.delete("<esc>");
    }
  }
  async getCommonKeys() {
...
var TabContainerManager = class {
  constructor() {
    this.initedEmitter = new import_coc22.Emitter();
  ...
  async register() {
    const bufnrs = await import_coc22.workspace.nvim.call("tabpagebuflist");
  ...
  async currentTabId() {
    ...
    return await import_coc22.workspace.nvim.call("coc_explorer#tab#current_id");
  }
  async currentTabMaxId() {
    await this.waitInited;
    return await import_coc22.workspace.nvim.call("coc_explorer#tab#max_id");
  }
...
var onEvent = (event, listener, thisArgs, disposables) => {
  const disposable = import_coc23.events.on(event, logger.asyncCatch(listener), thisArgs);
  const finalDisposable = import_coc23.Disposable.create(() => {
    if (typeof listener.cancel === "function") {
      listener.cancel();
    }
...
var InternalVimEvents = new HelperVimEvents(
  {
    BufDelete: {
      eventExpr: "BufDelete *",
      argExprs: ['+expand("<abuf>")']
...
var internalEvents = InternalVimEvents.events;
var cocListCloseEmitter = new import_coc23.Emitter();
function doUserAutocmdNotifier(name) {
  return Notifier.create(() => {
    import_coc23.workspace.nvim.call("coc_explorer#util#do_autocmd", [name], true);
  });
}
...
var BufManager = class {
  constructor(context2) {
    ...
    this.nvim = import_coc24.workspace.nvim;
  ...
  async registerEvents(context2) {
    ...
    const refreshBufModified = async (bufnr) => {
      ...
      const modified = await import_coc24.workspace.nvim.eval(
  ...
  async remove(fullpath, options) {
    if (options.directory) {
      return this.removePrefix(fullpath + import_path4.default.sep, options);
    } else {
  ...
  async replaceBufNode(bufNode, targetFullpath, options) {
    ...
    if (winids.length) {
      ...
      for (const winid of winids) {
        ...
        if (import_coc24.workspace.isVim) {
  ...
  modified(fullpath, options) {
    var _a;
    if (options.directory) {
      return this.modifiedPrefix(fullpath + import_path4.default.sep);
    } else {
    ...
    this.bufferNodes = compactI2(
      await Promise.all(
        content.split(/\n/).map(async (line) => {
          ...
          const fullpath = await import_coc24.workspace.nvim.call("expand", [
            `#${bufnr}:p`,
            1
          ]);
          return {
            ...
            bufname,
            fullpath,
            name: import_path4.default.basename(bufname),
...
async function getPresets(config4) {
  const presets = await import_coc25.workspace.nvim.eval(
...
var _Args = class {
  ...
  static async parse(strArgs, config4) {
    ...
    if (!preset) {
      import_coc26.window.showMessage(
...
var GlobalContextVars = class extends ContextVars {
  constructor() {
    super(...arguments);
    this.nvim = import_coc27.workspace.nvim;
  }
  async read() {
...
var ActionExplorer = class extends ActionRegistrar {
  constructor(owner) {
    super(owner);
    this.waitActionMutex = new import_coc28.Mutex();
  ...
  async doActionExp(actionExp, options = {}) {
    ...
    try {
      ...
      import_coc28.window.showMessage(
...
var GitBinder = class {
  ...
  bind(source) {
    ...
    return import_coc29.Disposable.create(() => {
      binding.refCount -= 1;
      if (binding.refCount === 0) {
        (0, import_coc29.disposeAll)(this.registerForSourceDisposables);
        this.registerForSourceDisposables = [];
      }
      if (this.refTotalCount === 0) {
        (0, import_coc29.disposeAll)(this.registerDisposables);
        this.registerDisposables = [];
      }
    });
  }
      ...
      ...["CocGitStatusChange", "FugitiveChanged"].map(
        (event) => internalEvents.on(event, async () => {
          this.reloadDebounceSubject.next({
            ...
            directory: import_coc29.workspace.cwd
          });
        })
      ),
      onEvent("BufWritePost", async (bufnr) => {
        ...
        if (fullpath) {
          const dirname = import_path5.default.dirname(fullpath);
          this.reloadDebounceSubject.next({
            sources: this.sources,
            directory: dirname
          });
  ...
  registerForSource(source) {
    return [
      source.events.on("loaded", async (node) => {
        const directory = "isRoot" in node ? source.root : node.expandable ? node.fullpath : node.fullpath && import_path5.default.dirname(node.fullpath);
        if (directory) {
...
var _GitCommand = class {
  ...
  async getRoot(filepath) {
    const stat = await fsStat(filepath).catch(() => void 0);
    const cwd = (stat == null ? void 0 : stat.isDirectory()) ? filepath : import_path6.default.dirname(filepath);
  ...
  parseStatusLine(gitRoot, line) {
    ...
    return [
      xFormat,
      yFormat,
      ...paths.map((p) => import_path6.default.join(gitRoot, p))
    ];
  }
  async status(root2, {
...
GitCommand.loadedEmitter = new import_coc30.Emitter();
...
var GitManager = class {
  ...
  async getGitRoot(directory) {
    ...
    if (root2)
      return root2;
    const parts = directory.split(import_path7.default.sep);
    const idx = parts.indexOf(".git");
    if (idx !== -1) {
      const root3 = parts.slice(0, idx).join(import_path7.default.sep);
      this.rootCache.set(directory, root3);
    } else {
      try {
        const gitRoot = await this.cmd.getRoot(directory);
        if (import_path7.default.isAbsolute(gitRoot)) {
          this.rootCache.set(directory, gitRoot);
        } else {
          import_path7.default.join(directory, gitRoot);
        }
      } catch (error) {
        logger.error(error);
        return;
  ...
  async reload(directory, options = {}) {
    ...
    if (root2) {
      ...
      statusRecord.forEach((status, fullpath) => {
        ...
        const relativePath = import_path7.default.relative(root2, fullpath);
        const parts = relativePath.split(import_path7.default.sep);
        for (let i = 1; i <= parts.length; i++) {
          const frontalPath = import_path7.default.join(
            root2,
            import_path7.default.join(...parts.slice(0, i))
          );
  ...
  bindColumn(source) {
    ...
    if (!enabled) {
      return import_coc31.Disposable.create(() => {
      });
    }
      ...
      const isIgnore = [...gitIgnores.entries()].some(
        ([ignorePath, gitIgnore]) => {
          if (gitIgnore === 1 /* file */) {
            ...
            const directoryPath = isDirectory ? fullpath + import_path7.default.sep : fullpath;
...
var OpenActionContext = class {
  constructor(explorer, source, cursorPosition, originalOpenByWinnr, getFullpath, quitOnOpenNotifier, explorerWinid) {
    ...
    this.nvim = import_coc32.workspace.nvim;
    ...
    this.openByWinnr = originalOpenByWinnr ?? (async (winnr) => {
      ...
      await this.openWrap(() => {
        ...
        if (import_coc32.workspace.isVim) {
          this.nvim.command("redraw", true);
        }
      });
    });
  ...
  async getEscapePath() {
    ...
    if (this.explorer.config.get("openAction.relativePath")) {
      path2 = await import_coc32.workspace.nvim.call("fnamemodify", [path2, ":."]);
    }
    return await import_coc32.workspace.nvim.call("fnameescape", [path2]);
  }
  async tryResize() {
...
function loadGlobalActions(action) {
  ...
  const { nvim } = import_coc33.workspace;
...
var FloatingWindow2 = class {
  ...
  async open(lines, highlights, options) {
    ...
    await this.win.open({
      ...
      initedExecute: ({ winid }) => {
        const scripts = [];
        if (import_coc34.workspace.isNvim) {
          scripts.push(`
...
var FloatingPreview = class {
  constructor(explorer) {
    ...
    this.nvim = import_coc35.workspace.nvim;
    ...
    this.disposables.push(
      explorer.events.on("first-open-post", () => {
        this.disposables.push(
          ...
          import_coc35.Disposable.create(() => {
            (0, import_coc35.disposeAll)(this.onHoverDisposables);
          })
        );
  ...
  registerOnHover(onHoverStrategy, delay = 300) {
    ...
    import_coc35.window.showMessage(`Preview ${onHoverStrategy} enabled`);
  }
  unregisterOnHover() {
    ...
    import_coc35.window.showMessage("Preview disabled ");
  }
  ...
  registerActions() {
    ...
    this.registerAction("content", async ({ node }) => {
      ...
      if (node.location) {
        ...
        location = import_coc35.Location.create(node.fullpath, import_coc35.Range.create(0, 0, 0, 0));
      } else {
        return;
      }
      const { uri, range } = location;
      if (await (0, import_isbinaryfile.isBinaryFile)(uri)) {
        import_coc35.window.showMessage("Preview content skip binary");
        return;
      }
      const doc = import_coc35.workspace.getDocument(uri);
  ...
  async getFloatOptions(lines) {
    const env = import_coc35.workspace.env;
  ...
  async previewNode(previewStrategy, source, node, nodeIndex) {
    ...
    if (!this.registeredPreviewActions[previewStrategy]) {
      import_coc35.window.showMessage(
...
var HighlightManager = class {
  ...
  linkGroup(groupName, targetGroup) {
    ...
    const commands5 = [`highlight default link ${group} ${targetGroup}`];
    const highlight = {
      group,
      commands: commands5
    };
  ...
  createGroup(groupName, hlCommand) {
    const group = `CocExplorer${groupName}`;
    const commands5 = [`highlight default ${group} ${hlCommand}`];
    const highlight = {
      group,
      commands: commands5
    };
  ...
  addHighlightsNotify(explorer, hlSrcId, highlights) {
    for (const hl of highlights) {
      ...
      explorer.buffer.highlightRanges(-1, hl.group, [
        import_coc36.Range.create(
          import_coc36.Position.create(hl.lineIndex, hl.start),
          import_coc36.Position.create(hl.lineIndex, hl.start + hl.size)
        )
      ]);
    }
  }
  bootHighlightSyntaxNotify() {
    ...
    for (const hl of this.highlights) {
      ...
      commands5.push(...hl.commands);
    }
    this.nvim.call("coc_explorer#util#execute_commands", [commands5], true);
  }
};
...
async function handleGrow(fullwidth, usedWidth, drawableList) {
  ...
  const spaceWids = divideVolumeBy(
    ...
    drawableList.map(
      (c) => {
        var _a;
        return c.type === "group" && ((_a = c.flexible) == null ? void 0 : _a.grow) ? c.flexible.growVolume ?? 1 : 0;
      }
    )
  );
  return compactI2(
...
async function handleOmit(fullwidth, usedWidth, drawableList) {
  ...
  const omitWids = divideVolumeBy(
    ...
    drawableList.map(
      (c) => {
        var _a;
        return c.type === "group" && ((_a = c.flexible) == null ? void 0 : _a.omit) ? c.flexible.omitVolume ?? 1 : 0;
      }
    ),
    drawableList.map((c) => {
      if (c.type === "content") {
...
async function registerMappings(context2, explorerManager) {
  ...
  for (const [mode, keys2] of keysModes) {
    ...
    for (const key of keys2) {
      ...
      context2.subscriptions.push(
        import_coc37.workspace.registerKeymap([mode], plugKey, async () => {
          const count = await import_coc37.workspace.nvim.eval("v:count");
  ...
  await import_coc37.workspace.nvim.call("coc_explorer#mappings#register", [mappings]);
}
async function executeMappings() {
  await import_coc37.workspace.nvim.call("coc_explorer#mappings#execute", [mappings]);
}
async function clearMappings() {
  await import_coc37.workspace.nvim.call("coc_explorer#mappings#clear", [mappings]);
}
...
var HelpPainter = class {
  ...
  async render() {
    import_coc38.workspace.nvim.pauseNotification();
    ...
    await import_coc38.workspace.nvim.resumeNotification();
  }
};
async function showHelp(explorer, source) {
  ...
  ["<esc>", "q", "?"].forEach((key) => {
    disposables.push(
      import_coc38.workspace.registerLocalKeymap(
        "n",
        key,
        async () => {
          (0, import_coc38.disposeAll)(disposables);
...
var HighlightExplorer = class {
  ...
  async bootSyntax() {
    ...
    const curWinnr = await import_coc39.workspace.nvim.call("winnr");
    if (winnr) {
      import_coc39.workspace.nvim.pauseNotification();
      if (winnr !== curWinnr) {
        import_coc39.workspace.nvim.command(`${winnr}wincmd w`, true);
      }
      hlGroupManager.bootHighlightSyntaxNotify();
      if (winnr !== curWinnr) {
        import_coc39.workspace.nvim.command(`${curWinnr}wincmd w`, true);
      }
  ...
  async gotoLineIndexNotifier(lineIndex, col) {
    ...
    return Notifier.create(() => {
      if (win) {
        ...
        if (import_coc40.workspace.isVim) {
          import_coc40.workspace.nvim.command("redraw", true);
        } else {
          import_coc40.workspace.nvim.command("redraw!", true);
        }
      }
...
async function extractHighlightsColor(highlightGroups) {
  const hlColorStrs = await import_coc41.workspace.nvim.call("coc_explorer#highlight#extract_colors", [
    highlightGroups
  ]);
...
var registerInternalColors = (disposables) => {
  hlGroupManager.watchColorScheme(disposables, async () => {
    ...
    const { nvim } = import_coc42.workspace;
    nvim.pauseNotification();
  ...
  clearParts(parts) {
    parts.forEach((part) => {
      ...
      if (typeof part !== "string") {
        ...
        (_a = part.modifiers) == null ? void 0 : _a.forEach((m) => this.clearColumn(m.column));
      }
    });
  }
...
var SourcePainters = class {
  ...
  async drawNodeLabeling(node, nodeIndex) {
    ...
    for (const part of this.getPainter(node.type).labelingParts) {
      ...
      const columns = [
        part.column,
        ...((_a = part.modifiers) == null ? void 0 : _a.map((m) => m.name === "&" ? m.column : "")) ?? []
      ].filter((c) => typeof c !== "string");
      const visibleColumns = await (0, import_p_filter.default)(
        columns,
        async (c) => {
          var _a2;
          return !((_a2 = c.drawHandle) == null ? void 0 : _a2.labelVisible) || await c.drawHandle.labelVisible({ node, nodeIndex });
        }
      );
      ...
      const isAllLabelOnly = visibleColumns.every(
        (c) => {
          var _a2;
          return (_a2 = c.drawHandle) == null ? void 0 : _a2.labelOnly;
        }
      );
    ...
    return await this.view.sourcePainters.drawPre(finalNodes, {
      draw: async () => {
        ...
        return Notifier.create(() => {
          ...
          if (import_coc43.workspace.isVim) {
            import_coc43.workspace.nvim.command("redraw", true);
          }
        });
      },
...
var RendererExplorer = class {
  constructor(view, explorer = view.explorer) {
    this.view = view;
    this.explorer = explorer;
    this.renderMutex = new import_coc44.Mutex();
  }
  async runQueue(fn) {
    ...
    await this.sync(async (r) => {
      ...
      await this.sourcePainters.drawPre(needDrawNodes, {
        draw: async () => {
          ...
          await this.source.events.fire("drawn");
          import_coc45.workspace.nvim.pauseNotification();
          this.setLinesNotifier(contents, startIndex, endIndex + 1).notify();
          this.source.highlight.addHighlightsNotify(highlightPositions);
          if (import_coc45.workspace.isVim) {
            import_coc45.workspace.nvim.command("redraw", true);
          }
          await import_coc45.workspace.nvim.resumeNotification();
        },
...
var ExplorerSource = class {
  constructor(sourceType, explorer) {
    ...
    this.nvim = import_coc46.workspace.nvim;
  ...
  get root() {
    return import_coc46.workspace.cwd;
  }
  set root(_root) {
  }
  static get enabled() {
...
var SourceManager = class {
  ...
  registerSource(name, source) {
    ...
    return import_coc47.Disposable.create(() => {
...
function loadBufferActions(action) {
  const { nvim } = import_coc48.workspace;
...
var ColumnRegistrar = class {
  ...
  registerColumn(type, name, createColumn) {
    ...
    return import_coc49.Disposable.create(() => {
...
var registerGitHighlights = (disposables) => {
  hlGroupManager.watchColorScheme(disposables, async () => {
    ...
    const { nvim } = import_coc50.workspace;
    nvim.pauseNotification();
...
var DiagnosticBinder = class {
  ...
  bind(source, type) {
    ...
    return import_coc51.Disposable.create(() => {
      var _a, _b;
      binding.refCount[type] -= 1;
      binding.refCount.total -= 1;
  ...
  registerForSource(source) {
    const reload = source.events.on("loaded", async (node) => {
      const directory = "isRoot" in node ? source.root : node.expandable ? node.fullpath : node.fullpath && import_path8.default.dirname(node.fullpath);
      if (directory) {
...
var DiagnosticManager = class {
  ...
  bindColumn(source, types) {
    ...
    return import_coc52.Disposable.create(() => {
      (0, import_coc52.disposeAll)(disposables);
    });
  }
  async reload(types) {
    const typeSet = new Set(types);
    ...
    for (const diagnostic of await import_coc52.diagnosticManager.getDiagnosticList()) {
      const uri = diagnostic.location.uri;
      const path2 = normalizePath(import_coc52.Uri.parse(uri).fsPath);
  ...
  reloadMixedErrors(errorPathCount) {
    ...
    for (const [fullpath, count] of errorPathCount) {
      const parts = fullpath.split(import_path9.default.sep);
      for (let i = 1; i <= parts.length; i++) {
        const frontalPath = parts.slice(0, i).join(import_path9.default.sep);
...
var Task = class extends import_events7.EventEmitter {
  ...
  start(cmd, args, cwds, excludePatterns) {
    ...
    for (const cwd of cwds) {
      ...
      const range = import_coc53.Range.create(0, 0, 0, 0);
      const hasPattern = excludePatterns.length > 0;
      process3.stderr.on("data", (chunk) => {
        console.error(chunk.toString("utf8"));
      });
      rl.on("line", (line) => {
        const file = import_path10.default.join(cwd, line);
        if (hasPattern && excludePatterns.some((p) => (0, import_minimatch2.default)(file, p))) {
          return;
        }
        const location = import_coc53.Location.create(import_coc53.Uri.file(file).toString(), range);
        this.emit("data", {
          label: line,
          location
        });
...
var config3 = import_coc53.workspace.getConfiguration("list.source.files");
...
var driveList = registerList({
  ...
  doHighlight() {
    const { nvim } = import_coc54.workspace;
    nvim.pauseNotification();
...
var explorerWorkspaceFolderList = registerList({
  ...
  async loadItems(fileSource) {
    return import_coc55.workspace.folderPaths.map((path2) => ({
      label: path2,
      data: {
        path: path2,
        callback: () => {
...
function loadFileActions(action) {
  ...
  action.addNodeAction(
    ...
    async () => {
      ...
      if (/^[A-Za-z]:[\\/]$/.test(file.root)) {
        ...
        file.root = import_path11.default.dirname(file.root);
  ...
  action.addNodesAction(
    ...
    async ({ nodes }) => {
      ...
      import_coc56.window.showMessage("Copy filepath to clipboard");
    },
  ...
  action.addNodesAction(
    ...
    async ({ nodes }) => {
      await file.copyToClipboard(
        nodes ? nodes.map((it) => import_path11.default.relative(file.root, it.fullpath)).join("\n") : file.root
      );
      import_coc56.window.showMessage("Copy relative filepath to clipboard");
    },
  ...
  action.addNodesAction(
    ...
    async ({ nodes }) => {
      await file.copyToClipboard(
        nodes ? nodes.map((it) => it.name).join("\n") : import_path11.default.basename(file.root)
      );
      import_coc56.window.showMessage("Copy filename to clipboard");
    },
  ...
  action.addNodeAction(
    ...
    async ({ node }) => {
      ...
      if (content.type === "none" || content.fullpaths.length <= 0) {
        import_coc56.window.showMessage("Copied or cut files is empty", "error");
        return;
      }
      ...
      if (content.type === "copy") {
        overwriteResult = await overwritePrompt(
          ...
          fullpaths.map((fullpath) => ({
            source: fullpath,
            target: import_path11.default.join(targetDir, import_path11.default.basename(fullpath))
          })),
          fsCopyFileRecursive
        );
...
${list}`
  ...
  action.addNodeAction(
    ...
    async ({ node, args }) => {
      ...
      const targetPath = import_path11.default.join(putTargetNode.fullpath, filename);
  ...
  action.addNodeAction(
    ...
    async ({ node, args }) => {
      ...
      const putTargetNode = file.getPutTargetNode(node);
      const targetPath = import_path11.default.join(putTargetNode.fullpath, directoryName);
      await overwritePrompt(
        "add directory",
        [
          {
  ...
  action.addNodeAction(
    ...
    async ({ node, args }) => {
      ...
      await file.searchByCocList(
        node.isRoot ? node.fullpath : import_path11.default.dirname(node.fullpath),
  ...
  action.addNodeAction(
    ...
    async ({ node }) => {
      ...
      await file.searchByCocList(import_path11.default.dirname(node.fullpath), {
        recursive: true,
        noIgnore: false,
        strict: false
      });
...
fileColumnRegistrar.registerColumn("root", "root", ({ source }) => ({
  draw() {
    return {
      drawNode(row) {
        row.add(import_path12.default.basename(source.root), { hl: fileHighlights.rootName });
      }
    };
  }
}));
...
var BuiltinIconLoader = class extends IconLoader {
  ...
  getFileIcon(target) {
    const { fullname, basename, extensions: extensions3 } = target;
    const extname = extensions3[extensions3.length - 1];
    if (hasOwnProperty14(nerdfont.filenames, basename)) {
  ...
  getDirectoryIcon(target) {
    ...
    if (hasOwnProperty14(nerdfont.dirnames, basename)) {
...
var NvimWebDeviconsLoader = class extends IconLoader {
  ...
  async loadIcons(targets) {
    ...
    for (const target of targets) {
      ...
      const result = await import_coc57.workspace.nvim.call("luaeval", [
  ...
  async loadIcons(targets) {
    ...
    await Promise.all(
      targets.map(async (target) => {
        ...
        const code = await import_coc58.workspace.nvim.call("WebDevIconsGetFileTypeSymbol", [
          target.fullname,
          false
        ]);
        if (!code) {
...
var NerdfontVimLoader = class extends IconLoader {
  async loadIcons(targets) {
    ...
    await Promise.all(
      targets.map(async (target) => {
        const code = await import_coc59.workspace.nvim.call("nerdfont#find", [
          target.fullname,
          target.isDirectory
        ]);
        if (!code) {
...
fileColumnRegistrar.registerColumn("child", "fullpath", () => ({
  draw() {
    return {
      drawNode(row, { node }) {
        if (node.directory) {
          row.add(displayedFullpath(node.fullpath) + import_path13.default.sep, {
...
var FileSource = class extends ExplorerSource {
  constructor() {
    ...
    this.view = new ViewSource(
      ...
      fileColumnRegistrar,
      {
        type: "root",
        isRoot: true,
        uid: this.helper.getUid(import_path14.default.sep),
        name: "root",
        fullpath: (0, import_os3.homedir)(),
        expandable: true,
        directory: true,
        readonly: true,
        executable: false,
  ...
  isHidden(filename) {
    ...
    const { basename, extensions: extensions3 } = getExtensions(filename);
    const extname = extensions3[extensions3.length - 1];
    return hiddenRules.filenames.includes(basename) || hiddenRules.extensions.includes(extname) || hiddenRules.patternMatches.some(
  ...
  async cd(fullpath) {
    ...
    if (cdCmd === "tcd") {
      if (import_coc60.workspace.isNvim || await nvim.call("exists", [":tcd"])) {
        await nvim.command(`tcd ${escapePath}`);
        import_coc60.window.showMessage(`Tab's CWD is: ${fullpath}`);
      }
    } else if (cdCmd === "cd") {
      await nvim.command(`cd ${escapePath}`);
      import_coc60.window.showMessage(`CWD is: ${fullpath}`);
    }
  }
        ...
        revealCallback: async (loc) => {
          ...
          await this.view.sync(async (r) => {
            const [, notifiers] = await this.revealNodeByPathNotifier(
              r,
              import_coc60.Uri.parse(loc.uri).fsPath
            );
            await Notifier.runAll(notifiers);
          });
        }
  ...
  async revealNodeByPathNotifier(renderer, path2, {
    ...
    const revealRecursive = async (path3, {
      ...
      if (path3 === startNode2.fullpath) {
        return startNode2;
      } else if (startNode2.directory && path3.startsWith(startNode2.fullpath + import_path14.default.sep)) {
      ...
      filenames.map(async (filename) => {
        try {
          ...
          const fullpath = normalizePath(
            import_path14.default.join(parentNode.fullpath, filename)
          );
          const stat = await fsStat(fullpath).catch(() => {
          });
          const lstat = await fsLstat(fullpath).catch(() => {
          });
          const executable2 = await fsAccess(fullpath, import_fs2.default.constants.X_OK);
          const writable = await fsAccess(fullpath, import_fs2.default.constants.W_OK);
          const readable = await fsAccess(fullpath, import_fs2.default.constants.R_OK);
          const directory = isWindows2 && /^[A-Za-z]:[\\/]$/.test(fullpath) ? true : stat ? stat.isDirectory() : false;
          const child = {
            type: "child",
...
var BookmarkDB = class {
  ...
  async load() {
    const dir = import_path15.default.dirname(this.filepath);
...
var BookmarkSource = class extends ExplorerSource {
  ...
  static get enabled() {
    return import_coc61.extensions.getExtensionState("coc-bookmark") === "activated";
  }
  async init() {
    this.disposables.push(
      internalEvents.on(
  ...
  async loadChildren(parentNode) {
    var _a;
    const extRoot = import_coc61.workspace.env.extensionRoot;
    const bookmarkPath = import_path16.default.join(
      extRoot,
    ...
    for (const [filepath, bookmarks] of Object.entries(data)) {
      ...
      for (const lnum of Object.keys(bookmarks).map((l) => Number(l)).sort((l1, l2) => l1 - l2)) {
        ...
        bookmarkNodes.push({
          ...
          fullpath,
          name: import_path16.default.basename(fullpath),
          lnum,
          location: import_coc61.Location.create(fullpath, import_coc61.Range.create(lnum, -1, lnum, -1)),
          line: bookmark.line,
...
var Explorer = class {
  constructor(explorerID, explorerManager, bufnr, borderBufnr, config4) {
    ...
    this.borderBufnr = borderBufnr;
    this.config = config4;
    this.nvim = import_coc62.workspace.nvim;
    this.contentWidth = 0;
  ...
  static genExplorerPosition(args, specialSize) {
    ...
    if (args.position.name !== "floating") {
      ...
      const [vimWidth, vimHeight] = [
        import_coc62.workspace.env.columns,
        import_coc62.workspace.env.lines - import_coc62.workspace.env.cmdheight
      ];
      if (width <= 0) {
  ...
  static async create(explorerManager, argValues, config4) {
    ...
    const [bufnr, borderBufnr] = await import_coc62.workspace.nvim.call("coc_explorer#open_explorer", [
    ...
    const setWidth = async (contentWidthType, contentWidth) => {
      if (contentWidth <= 0) {
        ...
        if (contentWidthType === "win-width") {
          ...
          contentBaseWidth = await import_coc62.workspace.nvim.eval("&columns");
        }
        if (contentBaseWidth) {
  ...
  async initRoot(argValues, rooter) {
    ...
    this.root_ = normalizePath(import_coc62.workspace.cwd);
  }
  async initArgs(args, rooter) {
    ...
    (0, import_coc62.disposeAll)(this.sources_ ?? []);
  ...
  async getSelectedOrCursorLineIndexes(mode) {
    ...
    if (mode === "v") {
      const range = await import_coc62.window.getSelectedRange("v");
      if (range) {
...
var RooterOpened = class {
  constructor(workspaceRoot, explorer) {
    ...
    this.roots = {
      ...
      cwd: () => import_coc63.workspace.cwd,
      sourceBuffer: () => {
        return getSourceBufFullpath();
      },
      reveal: (revealPath) => revealPath ? import_path17.default.dirname(revealPath) : void 0
    };
  }
  async resolveRootByPatterns(dir, patterns, bottomUp) {
    const ndir = normalizePath(dir);
    const parts = ndir.split(import_path17.default.sep);
    if (bottomUp) {
      while (parts.length > 0) {
        const dir2 = parts.join(import_path17.default.sep);
      ...
      for (const part of parts) {
        curr.push(part);
        const dir2 = curr.join(import_path17.default.sep);
        if (await inDirectory(dir2, patterns)) {
          return dir2;
        }
      }
  ...
  async resolveRootBy(revealPath, strategyStr) {
    ...
    if (rootStrategyList.includes(strategy)) {
      ...
      let dir = import_coc63.workspace.cwd;
      const sourceBufFullpath = await this.roots.sourceBuffer();
      if (sourceBufFullpath) {
        dir = sourceBufFullpath.endsWith(import_path17.default.sep) ? sourceBufFullpath : import_path17.default.dirname(sourceBufFullpath);
      }
      if (customRule) {
  ...
  async resolveBufFullpath() {
    ...
    return normalizePath(import_path17.default.dirname(fullpath));
  }
};
...
var ExplorerManager = class {
  constructor(context2, bufManager) {
    ...
    this.nvim = import_coc64.workspace.nvim;
    this.events = new HelperEventEmitter(logger);
    ...
    this.context.subscriptions.push(
      import_coc64.Disposable.create(() => (0, import_coc64.disposeAll)(this.explorers()))
    );
  }
  ...
  async open(argStrs) {
    ...
    const rooter = new Rooter(import_coc64.workspace.root);
...
var PresetList = class extends import_coc65.BasicList {
  constructor(nvim) {
    super(nvim);
...
function registerApi(id, execute) {
  return import_coc66.commands.registerCommand(
    id,
...
async function getExplorer(explorerFinder, explorerManager) {
  if (explorerFinder === "closest") {
    const winFinder = await WinLayoutFinder.create();
    const curWinid = await import_coc66.workspace.nvim.eval(
      "win_getid(winnr())"
    );
    if (curWinid <= -1) {
      return;
  ...
  context2.subscriptions.push(
    ...
    registerApi(
      ...
      async (filepath, isDirectory = false, isExpanded) => {
        const basename = import_path18.default.basename(filepath);
        ...
        return icons == null ? void 0 : icons[type].get(basename);
      }
    ),
    registerApi(
      "explorer.getIcons",
      async (paths) => {
        ...
        const targets = paths.map((it) => {
          const fullname = import_path18.default.basename(it.filepath);
...
var activate = (context2) => {
  ...
  const { nvim } = import_coc67.workspace;
  ...
  import_coc67.listManager.registerList(new PresetList(nvim));
  ...
  subscriptions.push(
    import_coc67.commands.registerCommand(
      "explorer",
      logger.asyncCatch((...args) => {
        explorerManager.open(args).catch(logger.error);
      })
    ),
    import_coc67.languages.registerCodeActionProvider(
      ["coc-explorer"],
  ...
  (async () => {
    ...
    await InternalVimEvents.register(context2);
    await registerRuntimepath(context2.extensionPath);
